...
public class UIChapSmartController : Controller
{
    ...
    {
        var merchantId = model.Settings.MerchantId?.Trim() ?? "";

        // SECURITY: Validate MerchantId format if provided
        if (!string.IsNullOrEmpty(merchantId) && !MerchantIdPattern.IsMatch(merchantId))
        {
            TempData["ErrorMessage"] = "Invalid Merchant ID format. It should start with mch_ followed by 16 hex characters (e.g., mch_a1b2c3d4e5f6a7b8).";
            return RedirectToAction(nameof(EditChapSmart), new { storeId });
        }

        // SECURITY: Validate API URL if changed from default
        var apiUrl = string.IsNullOrWhiteSpace(model.Settings.ApiUrl)
            ? "https://backend.chapsmart.com"
            : model.Settings.ApiUrl.Trim();

        if (!apiUrl.StartsWith("https://", StringComparison.OrdinalIgnoreCase) &&
            !apiUrl.StartsWith("http://localhost", StringComparison.OrdinalIgnoreCase))
        {
            TempData["ErrorMessage"] = "API URL must use HTTPS.";
            return RedirectToAction(nameof(EditChapSmart), new { storeId });
        }
